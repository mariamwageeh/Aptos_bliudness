...
import styles from './Getalldocbyadmin.module.css';
import axios from 'axios';
import { useState,useEffect } from 'react';
export default function Getalldocbyadmin() {
  ...
  //  const observationId=localStorage.getItem('observationId');
    ...
    const response = await axios.delete(
      `https://localhost:44389/api/admin/deletedoctor/${id}`,// Update the URL here
...
<div className='observe px-2 py-3 '>
      ...
      {/* <img className='w-100' src={`data:image/png;base64,${observe.profileImg}`} alt='err'/> */}
        <span className='fw-bold '>doctor name:{observe.username}</span>
        <h3 className='h6'>user Id:{observe.userId.split(' ').slice(0,2).join(' ')}</h3>
