...
import styles from './UpdateP.css';
...
export default function UpdateP() {
  ...
  async function getDoc() {
    try {
      let { data } = await axios.get(`https://localhost:44389/api/Patient/ShowProfile/${id}`, {
  ...
  const handleRegistrationSubmit = async (event) => {
    ...
    try {
      const response = await axios.put(
        `https://localhost:44389/api/Patient/UpdatePatientInfo/${id}`,
        formData,
        { headers: { Authorization: `Bearer ${token}` } }
      );
      setLoading(false);
